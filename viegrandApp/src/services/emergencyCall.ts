...
class EmergencyCallService {
	...
	// Thực hiện cuộc gọi tới một số bất kỳ (dùng cho gọi trong chat)
	async callNumber(targetPhone: string, displayName?: string): Promise<void> {
		try {
			const phone = (targetPhone || '').trim();
			if (!phone) {
				Alert.alert('Lỗi', 'Không tìm thấy số điện thoại người nhận.');
				return;
			}
			...
			const hasPermission = await this.requestCallPermission();
			if (!hasPermission) {
				if (showPermissionGuide) showPermissionGuide();
				else Alert.alert('Quyền gọi điện bị từ chối', 'Vui lòng cấp quyền gọi điện trong Cài đặt.');
				return;
			}

			// 2. Hỗ trợ gọi
			const canCall = await this.canMakePhoneCall(phone);
			if (!canCall) {
				Alert.alert('Không thể gọi', 'Thiết bị không hỗ trợ thực hiện cuộc gọi.');
				return;
			}
			...
			Alert.alert(
				'Gọi điện',
				`Bạn có muốn gọi cho ${name}?\nSố: ${phone}`,
				[
					{ text: 'Hủy', style: 'cancel' },
					{ text: 'Gọi', style: 'destructive', onPress: async () => {
						try {
							if (Platform.OS === 'android') {
								await NativeModules.DirectCallModule.makeDirectCall(phone);
							} else {
